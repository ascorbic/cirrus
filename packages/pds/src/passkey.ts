...
export async function initPasskeyRegistration(
	...
	await accountDO.account().savePasskeyToken(
		token,
		options.challenge,
		expiresAt,
		name,
...
export async function getRegistrationOptions(
	...
	const storage = await accountDO.account().consumePasskeyToken(token);
	if (!storage) {
		return null;
	}
	...
	const existingPasskeys = await accountDO.account().listPasskeys();
...
export async function completePasskeyRegistration(
	...
	const tokenData = await accountDO.account().consumePasskeyToken(token);
	if (!tokenData) {
	...
	try {
		...
		await accountDO.account().savePasskey(
			credential.id,
...
export async function getAuthenticationOptions(
	...
	const passkeys = await accountDO.account().listPasskeys();
	...
	await accountDO.authStore().saveWebAuthnChallenge(options.challenge);
...
export async function verifyPasskeyAuthentication(
	...
	try {
		...
		const challengeValid =
			await accountDO.authStore().consumeWebAuthnChallenge(challenge);
		if (!challengeValid) {
			return { success: false, error: "Invalid or expired challenge" };
		}

		// Get the passkey from storage
		const passkey = await accountDO.account().getPasskey(response.id);
		if (!passkey) {
		...
		// Update the counter
		await accountDO.account().updatePasskeyCounter(
			response.id,
...
export async function deletePasskey(
	...
	return accountDO.account().deletePasskey(credentialId);
}
