...
Adds `normalizeRecordLinks()` to convert `$link` → CID and `$bytes` →
Uint8Array on all record write paths (createRecord, putRecord, applyWrites).
Also adds `$bytes` → Uint8Array serialization on the read path for
completeness.
